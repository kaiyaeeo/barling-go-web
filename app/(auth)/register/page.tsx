    ...
    return (
        ...
        <div className={`hidden lg:flex lg:w-5/12 flex-col justify-between p-12 transition-colors duration-300 ${isSeller ? "bg-[#1a3a2a]" : "bg-[#6EB8BB]"}`}>
            ...
            {isSeller ? (
                ...
                <h2 className="text-4xl font-black text-white leading-tight mb-4">Bergabung<br />sebagai Mitra<br />UMKM!</h2>
                <p className="text-green-200/80 text-base leading-relaxed max-w-sm">Daftarkan toko UMKM-mu dan jangkau lebih banyak pelanggan dari seluruh Indonesia.</p>
                </>
            ) : (
                <>
                <div className="w-14 h-14 rounded-2xl bg-white/15 flex items-center justify-center mb-6">
                ...
                <h2 className="text-4xl font-black text-white leading-tight mb-4">Mulai<br />petualanganmu!</h2>
                <p className="text-green-100/80 text-base leading-relaxed max-w-sm">Bergabung dengan ribuan wisatawan yang sudah menjelajahi keindahan Barlingmascakep.</p>
                </>
            )}
            </div>
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white overflow-y-auto">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-1">{isSeller ? "Daftar sebagai Seller" : "Buat akun baru"}</h1>
            <p className="text-sm text-gray-500 mb-8">Sudah punya akun? <Link href={`/login${isSeller ? "?mode=seller" : ""}`} className="text-[#6EB8BB] font-semibold hover:underline">Masuk di sini</Link></p>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
                ...
                <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Nama lengkap kamu" required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#6EB8BB]/30 focus:border-[#6EB8BB] transition-all" />
                </div>
                {isSeller && (
                <>
                    <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Nama Toko / UMKM *</label>
                    <input type="text" value={shopName} onChange={(e) => setShopName(e.target.value)} placeholder="Contoh: Batik Banyumas Bu Siti" required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#6EB8BB]/30 focus:border-[#6EB8BB] transition-all" />
                    </div>
                    <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Jenis Usaha</label>
                    <select value={shopType} onChange={(e) => setShopType(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#6EB8BB]/30 focus:border-[#6EB8BB] bg-white">
